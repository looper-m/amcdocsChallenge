...
public class ParseTable {
	...
	void getBodyElements() {
		...
		HashMap<IBodyElement, List<IBodyElement>> allItems = new HashMap<IBodyElement, List<IBodyElement>>();
		String currentStyle, lastStyle = "", lastToLastStyle = "";
		IBodyElement lastElement = null, currentElement = null;

		try {
			FileInputStream fis = new FileInputStream(
					"C:/KIAS/UPS/KIAS_CSM_SD_DB_V992 CSM EN Data Model - Entities1.docx");
			...
			for (IBodyElement para : xwpfElement) {
				...
				if (para instanceof XWPFParagraph) {
					...
					if (currentStyle != null) {
						...
						if ((currentStyle.equals("Heading1"))) {
							...
							lastElement = para;
						} else if ((currentStyle.equals("Heading2") || currentStyle.equals("BodyText"))) {
			...
			for (Entry<IBodyElement, List<IBodyElement>> entry : allItems.entrySet()) {
				...
				while (itr1.hasNext()) {
					...
					System.out.println(temp_para.getText());
				}System.out.println("**********************************************************************************");
			}
